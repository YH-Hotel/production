...
const ShootingRange = require("../../model/shootingRange.model");

const ShootingRangePOSTAPI = async (req, res, next) => {
  try {
    let {
      hotelName,
      city,
...
const ShootingRangeGETAPI = async (req, res, next) => {
  try {
    const findData = await ShootingRange.find()
      .sort({ createdAt: -1 })
      .limit(8);
    return successHandler(res, "ShootingRange fetched successfully", 200, {
      findData,
    });
  } catch (error) {
    console.log("Error", error);
...
const ShootingRangeGETByIdAPI = async (req, res, next) => {
  try {
    const findData = await ShootingRange.findOne({ _id: req.query._id });
...
const ShootingRangeUPDATEAPI = async (req, res, next) => {
  try {
    ...
    const findData = await ShootingRange.findOne({ _id });
    if (!findData) {
      return next(errorHandler(res, "ShootingRange not found", 404));
    }

    await ShootingRange.findByIdAndUpdate(findData._id, {
      img,
      stars,
      reviews_count,
      title,
    ...
    return successHandler(res, "ShootingRange updated successfully", 200);
  } catch (error) {
    console.log("Error", error);
    return next(errorHandler(res, error.message, 500));
  }
...
const ShootingRangeDELETEAPI = async (req, res, next) => {
  try {
    let { _id } = req.body;
    if (!_id) {
      return next(errorHandler(res, "Please enter all fields", 400));
    ...
    await ShootingRange.findByIdAndDelete(findData._id);
    return successHandler(res, "ShootingRange deleted successfully", 200);
  } catch (error) {
    console.log("Error", error);
    return next(errorHandler(res, error.message, 500));
  }
